...
export default function RestaurantRequirements() {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiFetch(`/api/restaurant/meal-requirements?date=${date}`)
    ...
    return (
      <div className="animate-fade-in">
        ...
        <div className="flex flex-col items-center justify-center text-center py-16">
          ...
          <h3 className="text-lg font-semibold text-surface-900">Failed to load requirements</h3>
          <p className="text-surface-500 mt-1">{error}</p>
          <button onClick={() => setDate(date)} className="mt-4 text-brand-600 font-medium hover:text-brand-700">
            Try again
          </button>
        </div>
      </div>
  ...
  const summaryCards = [
    { label: 'Confirmed', value: totalConfirmed, icon: <CheckIcon />, color: 'success', bg: 'bg-success-50', text: 'text-success-600', iconBg: 'bg-success-100' },
    { label: 'Declined', value: totalDeclined, icon: <XIcon />, color: 'danger', bg: 'bg-danger-50', text: 'text-danger-600', iconBg: 'bg-danger-100' },
    { label: 'No Response', value: totalPending, icon: <ClockIcon />, color: 'warning', bg: 'bg-warning-50', text: 'text-warning-600', iconBg: 'bg-warning-100' },
    { label: 'Total', value: totalEmployees, icon: <UsersIcon />, color: 'brand', bg: 'bg-brand-50', text: 'text-brand-600', iconBg: 'bg-brand-100' },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {reqs.length === 0 ? (
        <EmptyState
          ...
          description={`No meal requirements available for ${new Date(date + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}`}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {reqs.map((req, idx) => {
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="min-w-0 flex-1">
                    <h3 className="text-sm font-semibold text-surface-900 truncate">{req.organization_name}</h3>
                    <p className="text-xs text-surface-400 mt-0.5">
                      {req.confirmed} of {req.total_employees} confirmed
                    </p>
                  </div>
                  <span className="text-lg font-bold text-success-600 tabular-nums shrink-0 ml-3">{req.confirmed}</span>
                </div>

                {/* Mini Bar */}
                <div className="w-full h-2.5 rounded-full bg-surface-100 overflow-hidden flex">
                  {req.confirmed > 0 && (
                    <div
                ...
                <div className="flex items-center gap-3 mt-3">
                  {req.confirmed > 0 && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[11px] text-surface-500">{req.confirmed}</span>
                    </div>
                  )}
                  {req.declined > 0 && (
                    <div className="flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-danger-400" />
                      <span className="text-[11px] text-surface-500">{req.declined}</span>
                    </div>
                  )}
                  {req.pending > 0 && (
                    <div className="flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-warning-400" />
                      <span className="text-[11px] text-surface-500">{req.pending}</span>
                    </div>
                  )}
                </div>
